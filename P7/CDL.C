...
void searchNode(struct Node* last, int value) {
    struct Node* p;
    int found = 0;
    ...
    do {
        if (p->data == value) {
            printf("\nEl valor %d fue encontrado en la lista.", value);
            found = 1;
            break;
        }
    ...
    if (!found) {
        printf("\nEl valor %d no esta presente en la lista", value);
    }
}
